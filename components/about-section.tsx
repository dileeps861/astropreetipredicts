...
export function AboutSection({
  ...
  return (
    ...
    >
      <div className="material-box-strong rounded-[1.75rem] p-3 sm:p-4">
        {profileImageUrl ? (
          <div
            role="img"
            aria-label={profileName || "Astrology profile"}
            className="material-raised aspect-[4/5] w-full rounded-[1.35rem] border border-gold/15 bg-cover bg-center"
            style={{ backgroundImage: `url(${profileImageUrl})` }}
          />
        ) : (
          <div className="material-box flex aspect-[4/5] flex-col justify-between rounded-[1.35rem] p-6">
            <Image
      ...
      <div className="lg:pl-10">
        {location ? (
          <div className="material-chip mb-5 inline-flex items-center rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-starlight/70">
            Based in {location}
          </div>
        ) : null}
        <SectionHeading
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="material-box rounded-2xl p-6"
            >
...
function AboutDetailCard({
  ...
  return (
    <div
      className={`material-raised relative overflow-hidden rounded-2xl border p-5 ${style.className}`}
    >
      <div className="absolute right-3 top-3 h-16 w-16 rounded-full bg-white/45 blur-2xl" />
      <div className="relative flex items-start gap-4">
        <span className="material-chip grid h-10 w-10 shrink-0 place-items-center rounded-full text-sm font-semibold text-gold">
          {style.icon}
        </span>
        <div>
